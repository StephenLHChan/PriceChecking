...
const { AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, NODE_ENV, AWS_REGION } =
	process.env;
...
export function dynamoCreateItem({
	entity,
	attributes,
}: DynamoCreateItemParams) {
	...
	do {
		...
		if (filter) {
			...
			data = data.filter((datum) => {
				return filter.filters[filterMethod]((f) =>
					`${datum[f.property]}`
						.toLowerCase()
		...
		latestScanResult.LastEvaluatedKey &&
		pageIndex < maxPages &&
